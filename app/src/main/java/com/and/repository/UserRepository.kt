...
class UserRepository(private val application: Application) {
    ...
    suspend fun observeUser(
        ...
        try {
            val userInfoJob = CompletableDeferred<UserDataModel>()
            val drugInfoJob = CompletableDeferred<MutableList<DrugDataModel>>()
            val timeLineInfoJob = CompletableDeferred<HashMap<String, MutableList<TimeLineDataModel>>>()
            val warningInfoJob = CompletableDeferred<MutableList<String>>()
            ...
            userRef.child("category")
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        ...
                        drugInfoJob.complete(categories.sortedBy { it.creationTime }
                            .toMutableList())
                    }

                    override fun onCancelled(error: DatabaseError) {
                        Log.e(TAG, "DatabaseError: $error")  // 로그 추가
                        drugInfoJob.complete(mutableListOf<DrugDataModel>())
                    }
                })
            ...
            userRef.child("timeLine")
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        ...
                        if (snapshot.exists()) {
                            timeLines =
                                snapshot.getValue<HashMap<String, MutableList<TimeLineDataModel>>>()!!
                        }
                        timeLineInfoJob.complete(timeLines)
                    }

                    override fun onCancelled(error: DatabaseError) {
                        Log.e(TAG, "DatabaseError: $error")  // 로그 추가
                        timeLineInfoJob.complete(hashMapOf<String, MutableList<TimeLineDataModel>>())
                    }
                })
            ...
            userRef.child("warning")
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        ...
                        warningInfoJob.complete(warnings)
                    }

                    override fun onCancelled(error: DatabaseError) {
                        Log.e(TAG, "DatabaseError: $error")  // 로그 추가
                        warningInfoJob.complete(mutableListOf<String>())
                    }
                })

            userInfo.postValue(userInfoJob.await())
            drugInfos.postValue(drugInfoJob.await())
            timeLineInfos.postValue(timeLineInfoJob.await())
            warningInfos.postValue(warningInfoJob.await())
